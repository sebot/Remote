...
/**
 * Class RemoteBackend - this class will change the WordPress
...
class RemoteBackend
{
     ...
     * 
     */
    public function __construct(RemoteAssets $assets)
    {
        // check if classic dashboard is desired
        $uiEnabled       = get_field('use_nice_dashboard', 'option');
        if (true === $uiEnabled) {
            add_action('login_enqueue_scripts', [$this, 'loadRemoteLoginUI']);
    ...
    /**
     * Load the remote UI which will change the way
     * the WordPress admin backend works UI/UX wise.
     * 
     * @return void
     */
    public function loadRemoteLoginUI(): void
    {
        wp_enqueue_style('remote-backend', Remote::$themeAssetUri . '/css/login.css');
